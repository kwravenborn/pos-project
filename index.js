...
//         return dbConnection.execute('SELECT `email` FROM `employee` WHERE `email`=?', [value])
//         .then(([rows]) => {
//             if(rows.length > 0){
...
//             dbConnection.execute("INSERT INTO `employee`(`name`,`email`,`password`) VALUES(?,?,?)",[user_name,user_email, hash_pass])
//             .then(result => {
...
app.post('/', ifLoggedin, [
    ...
    if(validation_result.isEmpty()){
        ...
        dbConnection.execute("SELECT * FROM `employee` WHERE `email`=?",[user_email])
        .then(([rows]) => {
